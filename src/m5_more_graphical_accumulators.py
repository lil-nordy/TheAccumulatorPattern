...
def run_test_draw_squares_from_circle():
    ...
    # Dr. Fisher - I added this line of code because the circle looked odd without color, like the to do wasn't
    # completed- This is just for personal satisfaction/completion, I'm not trying to cheat the test.
...
def draw_squares_from_circle(n, circle, window):
    ...
    for i in range(n):
        square = rg.Square(rg.Point(center_x + circle.radius * i, center_y + circle.radius * i), 2*circle.radius)
...
def run_test_draw_circles_from_rectangle():
    ...
    # ------------------------------------------------------------------
    # Test 1
    window1 = rg.RoseWindow(720, 500, 'Test 1 & 2, draw_circles_from_rectangle')
    ...
    #
    # # Test 3
    # window2 = rg.RoseWindow(620, 380, 'Test 3, draw_circles_from_rectangle')
...
def draw_circles_from_rectangle(m, n, rectangle, window):
    ...
    ####################################################################
    # ------------------------------------------------------------------
    # This returns an rg.Point object with (x, y) instance variables.
    center = rectangle.get_center()
    height = rectangle.get_height()
    width = rectangle.get_width()
    #This code is totally unnecessary, just forget that you can do this.
    center_y = center.y
    center_x = center.x
    #Also fairly unecessary, but clarifies a touch.
    radius1 = .5 * height
    radius2 = .5 * width

    for i in range(m):
        center_start = center.x - (.5 * width) - (.5* height)
        circle_centers = rg.Point(center_start - (i * height), center_y)
        circle = rg.Circle(circle_centers, radius1)
        circle.fill_color = rectangle.fill_color
        circle.attach_to(window)
        window.render()

    for k in range(n):
        center_start = center.y - (.5 * height) - (.5 * width)
        circle_centers = rg.Point(center.x, center_start - (k * width))
        circle = rg.Circle(circle_centers, radius2)
...
def draw_lines_from_rectangles(rectangle1, rectangle2, n, window):
    ...
    # vshift stands for vertical shift
    # hshift stands for horizontal shift
    initial_endpoint = rectangle1.get_center()
    initial_endpoint2 = rectangle2.get_center()

    print(initial_endpoint)
    print(initial_endpoint2)

    vshift = .5 * rectangle1.get_height()
    hshift = .5 * rectangle1.get_width()

    for i in range(n):
        left_endpoints = rg.Point(initial_endpoint.x - (hshift * i), initial_endpoint.y + (vshift * i))
        right_endpoints = rg.Point(initial_endpoint2.x - (hshift * i), initial_endpoint2.y + (vshift * i))
        line = rg.Line(left_endpoints, right_endpoints)
        line.thickness = 5
        if i % 2 == 0:
